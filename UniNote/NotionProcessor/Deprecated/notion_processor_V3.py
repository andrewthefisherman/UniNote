                ...
                if block_type in rich_text_block_types:
                    ...
                    for i, rt in enumerate(rich_texts):
                        ...
                        if link and 'wikipedia.org' in link.get('url', ''):
                            ...
                            if text_content and text_content[-1] in self.superscript_mapping.values():
                            ...
                            if expect_superscript:
                                ...
                                if text_content and text_content[0] in self.superscript_mapping.values():
...
If no time is specified, do not add time, only include the date. If time is included, format it using the template (THH:MM) and add the +02:00 timezone offset. For relative time terms, refer to "{today}". If no start is given, default to "{today}". If end time is given but no start time is given, default to midnight. Output must strictly be the formatted string. If the sentence doesn't mention dates or doesn't make sense, respond "null".'''
    ...
    def process_custom(self, text: str) -> List[Dict[str, Any]]:
        """
        Processa gli elementi custom nel testo utilizzando le regex specificate e restituisce una lista di rich text per Notion.
        Gli elementi custom includono:
            - url(something) o link(something): link personalizzati
            - wik(something), wiki(something), wikipedia(something): link a Wikipedia
        """
        custom_elements = []
        # Pattern per gli elementi custom
        pattern = r'(url|link|wik|wiki|wikipedia)\(([^)]+)\)'
        matches = list(re.finditer(pattern, text))
        last_end = 0
        if not matches:
            # Nessun elemento custom, ritorna il testo come è
        ...
        for match in matches:
            ...
            # Aggiungi il testo prima del custom element
            if start > last_end:
                custom_elements.append({
                    'type': 'text',
                    'text': {
            ...
            if keyword in ['wik', 'wiki', 'wikipedia']:
                try:
                    ...
                    language = 'en'
                # Processa il link a Wikipedia
                wiki_response = self.check_wikipedia_page(content, language=language)
                if wiki_response:
                    # Gestione avanzata per wiki_response come lista
                    if isinstance(wiki_response, list):
                        context = self.string_window_cut(text, content, wrap='wiki(keyword)')
                        if context:
                            chat_history = [
                                f'user: "{context}".\n\nThis is a paragraph. Identify the wiki text entry. Once you identified it, I will give you some links and you will have to decide which link to assign, based on the paragraph context.',
                                f'chatbot: The wiki text entry is wiki({content}). Provide the links and I will choose which link could correspond to the wiki text entry, based on the context.'
                            ]
                            prompt = f'Here are the links: "{wiki_response}".\nBased on the context, which of these links do you think corresponds to the text entry? Respond strictly only with a number, corresponding to the position of the item in the list you choose.'
                            chosen_wiki_url_index, _ = self.cohere_request(prompt, chat_history)
                            try:
                                wiki_url = wiki_response[int(chosen_wiki_url_index.strip()) - 1]
                            except (ValueError, IndexError):
                                wiki_url = wiki_response[0]
                        else:
                            prompt = f'Here are some wiki links: "{wiki_response}".\nWhich of these links do you think corresponds to the text entry "{content}"? Respond strictly only with a number, corresponding to the position of the item in the list you choose.'
                            chosen_wiki_url_index, _ = self.cohere_request(prompt)
                            try:
                                wiki_url = wiki_response[int(chosen_wiki_url_index.strip()) - 1]
                            except (ValueError, IndexError):
                                wiki_url = wiki_response[0]
                    ...
                    for pairs in self.wiki_footnote_pairs:
                        if pairs[2] == wiki_url:
                            ...
                            self.wiki_footnote_pairs.append([content, superscript_number, wiki_url])
                            break

                    # Gestione dei superscript per le note a piè di pagina
                ...
                # Processa il link personalizzato
                ...
                # Gestisci altri custom elements se necessario
                raise SyntaxError(
        ...
        try:
            ...
            if not content_to_process:
                ...
                if wiki_footnote == 'true':
                    ...
                    logger.info(f'Wiki Footnote created. I\'m appending the wikipairs {self.wiki_footnote_pairs}')
