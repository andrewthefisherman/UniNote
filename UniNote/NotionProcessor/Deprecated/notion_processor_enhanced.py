...
class NotionProcessor:
    ...
    def resolve_link(self, partial_url: str) -> str:
        """
        Resolves a partial URL to an absolute URL by adding a default scheme and '.com' TLD if missing,
        validates the URL by checking its accessibility, and returns the absolute URL if valid.

        Args:
            partial_url (str): The partial URL to resolve (e.g., "youtube" or "youtube.com").

        Returns:
            str: The validated absolute URL (e.g., "https://youtube.com").

        Raises:
            NotionProcessorError: If the URL is invalid or inaccessible after validation attempts.
        """
        ...
        try:
            if not parsed.scheme:
                ...
                if not re.search(r'\.[a-z]{2,}$', domain, re.IGNORECASE):
                    domain += '.com'
                    logger.info(f"No TLD detected. Appended '.com' to domain: {domain}")

                absolute_url = f"{scheme}://{domain}{path}"
                logger.info(f"No scheme detected. Formatted to absolute URL: {absolute_url}")
            else:
                absolute_url = partial_url
                logger.info(f"Scheme detected. Using absolute URL as is: {absolute_url}")

            # Validate the URL with retries
            max_retries = 3
            retry_delay = 3  # seconds
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"Website validation for {absolute_url}: Valid (Status {response.status_code})")
                        return absolute_url
                    elif 300 <= response.status_code < 400:
                        # Handle redirects
                        logger.info(f"Website validation for {absolute_url}: Redirected (Status {response.status_code})")
                        absolute_url = response.url
                        logger.info(f"Redirected to: {absolute_url}")
                        # Continue the loop to validate the new URL
                        continue
                    elif response.status_code == 403:
                        # Treat 403 as valid but access restricted
                        logger.warning(f"Access forbidden when validating URL '{absolute_url}'. HTTP Status: {response.status_code}")
                        return absolute_url
                    else:
                        logger.warning(f"Access forbidden when validating URL '{absolute_url}'. HTTP Status: {response.status_code}")
                        break  # Do not retry for other non-200 status codes
                except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                    logger.error(f"Attempt {attempt} - Connection error during website validation for {absolute_url}: {e}")
                    if attempt < max_retries:
                        logger.info(f"Retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                        continue  # Retry the request
                    else:
                        logger.error(f"Website validation failed for {absolute_url} after {max_retries} attempts.")
                except requests.exceptions.RequestException as e:
                    logger.error(f"Request exception during website validation for {absolute_url}: {e}")
                    break  # Do not retry for other request exceptions

        except Exception as e:
            raise NotionProcessorError(
                message=f"The URL '{absolute_url}' is invalid or inaccessible.",
                location="resolve_link",
                troubleshoot="Verify the URL format and ensure the website is reachable.",
                status_code=400
            )
            ...
            if metadata_to_process:
                    ...
                    for header in headers_split:
                        ...
                        for pattern, replacement in zip(patterns, replacements):
                            if re.search(pattern, header):
                                ...
                                formatted_headers_list.append(formatted_header)
                ...
                for header in formatted_headers_list:
                    ...
                    if header.startswith('<tit>'):
                        if not title:
                            title = option_name
                            logger.debug(f"Title set to: {self.title}")
                        ...
                        properties = self.handle_select_property(properties, property_name, option_name, self.type_lists, property_options)

                    elif header.startswith('<msl>'):
                        properties = self.handle_multi_select_property(properties, property_name, option_name, self.type_lists, property_options)
    ...
    def assign_links_to_superscripts(self, page_id: str, links: list):
        updated_blocks_ids = []
        ...
        link_index = 0  # Index for the links list
        expect_superscript = False  # Flag to expect a superscript after a Wikipedia link

        for block in blocks['results']:
            if block['type'] == 'paragraph':
                paragraph = block['paragraph']
                rich_texts = paragraph['rich_text']
                superscripts_updated_in_block = 0  # Initialize the counter for each block

                new_rich_texts = []
                for i, rt in enumerate(rich_texts):

                    if rt.get("type") != "text":
                        # Add the rich_text without modifications if it's not of type 'text'
                        new_rich_texts.append(rt)
                        logger.debug(f"Skipping rich_text {i} of type {rt.get('type')}.")
                        continue

                    text_content = rt.get('text').get('content')
                    link = rt.get('text', {}).get('link', None)

                    # Debug: Show current content and link
                    logger.debug(f"Processing rich_text {i}: '{text_content}' with link: {link.get('url','') if link else 'No Link'}")

                    # Check if the text has a link to Wikipedia
                    if link and 'wikipedia.org' in link.get('url', ''):
                        # Verify if the last character of the text is a superscript without space
                        if text_content and text_content[-1] in self.superscript_mapping:
                            superscript_char = text_content[-1]
                            pre_text = text_content[:-1]  # Text before the superscript

                            # Add the text before the superscript
                            if pre_text:
                            ...
                            # Assign the link to the superscript character
                            if link_index >= len(links):
                                logger.error("Not enough links in the list to assign to all superscript numbers.")
                                raise InputError(
                                    'Not enough links in the list to assign to all superscript numbers.',
                                    location='assign_links_to_superscripts',
                                    troubleshoot='Ensure the links list is sufficient for the superscripts found.'
                                )

                            link_url = links[link_index]
                            new_rich_texts.append({
                                "type": "text",
                                "text": {
                                    "content": superscript_char,
                                    "link": {
                                        "url": link_url
                                    }
                                },
                            ...
                            logger.info(f"Assigned link {link_url} to character {superscript_char}.")
                            link_index += 1
                            superscripts_updated_in_block += 1  # Increment the counter
                        else:
                            # If there's no superscript at the end, set the flag to expect the next superscript
                            expect_superscript = True
                            new_rich_texts.append(rt)
                            logger.debug("Wikipedia link found, expecting next superscript.")
                    else:
                        if expect_superscript:
                            # Verify if the text starts with a superscript without space
                            if text_content and text_content[0] in self.superscript_mapping:
                                superscript_char = text_content[0]
                                # Assign the link to the superscript
                                if link_index >= len(links):
                                    logger.error("Not enough links in the list to assign to all superscript numbers.")
                                    raise InputError(
                                        'Not enough links in the list to assign to all superscript numbers.',
                                ...
                                # Remove the superscript character from the original text
                                remaining_text = text_content[1:]
                                if remaining_text:
                                    new_rich_texts.append({
                                        "type": "text",
                                        "text": {
                                ...
                                new_rich_texts.append(rt)
                            expect_superscript = False  # Reset the flag
                        else:
                            # No special condition, add the rich_text as is
                            new_rich_texts.append(rt)
                ...
                if superscripts_updated_in_block > 0:
                    ...
                    # Append the block ID once for each superscript updated
                    for _ in range(superscripts_updated_in_block):
                        updated_blocks_ids.append(block['id'])

        logger.info("Link assignment to superscripts completed.")
        return updated_blocks_ids

    def update_anchor_links(self, parent_block_id: str, links: list):
        updated_blocks_ids = []
        ...
        for block in response['results']:
            if block['type'] == 'numbered_list_item':
                ...
                for rt in rich_texts:
                    ...
                    # Debug: Show current rich_text content and link
                    logger.debug(f"Processing block ID {block['id']}: '{text_content}' with link: {link.get('url','') if link else 'No Link'}")

                    # Find all "⚓" characters in the text
                    if "⚓" in text_content:
                        # Split the text by "⚓" to isolate it
                        ...
                        for i, part in enumerate(parts):
                            ...
                            if i < len(parts) - 1:
                                # Add the "⚓" with the corresponding link
                                if link_index >= len(links):
                                    logger.error("Not enough links in the list to assign to all anchor emojis.")
                                    raise InputError(
                                        'Not enough links in the list to assign to all anchor emojis.',
                                        location='update_anchor_links',
                                        troubleshoot='Ensure the links list is sufficient for the anchor emojis found.'
                                    )
...
If no time is specified, do not add time, only include the date. If time is included, format it using the template (THH:MM) and add the +02:00 timezone offset. For relative time terms, refer to "{today}". If no start is given, default to "{today}". If end time is given but no start time is given, default to midnight. Output must strictly be the formatted string. If the sentence doesn't mention dates or doesn't make sense, respond "null".'''
    ...
    def notion_api_children(self, content_to_process: List[str]) -> List[Dict[str, Any]]:
        ...
        Args:
            content_to_process (List[str]): List of content strings to process.

        Returns:
            List[Dict[str, Any]]: List of Notion block dictionaries.
        
        Raises:
            NotionProcessorError: For unrecognized markers or unexpected errors.
        """
        children = []
        ...
        for element in content_to_process:
            ...
            try:
                if element.startswith('<hd1>'):
                    ...
                    formatted_element = re.sub(r'(ieq|eq|equation)\(([^)]+)\)', r'$<ieq>\2$<pgr>', element)
                    formatted_element = re.sub(r'(url|link)\(([^)]+)\)', r'$<url>\2$<pgr>', formatted_element)
                    formatted_element = re.sub(r'(icd|cod|code)\(([^)]+)\)', r'$<icd>\2$<pgr>', formatted_element)
                    formatted_element = re.sub(r'(wik|wiki|wikipedia)\(([^)]+)\)', r'$<wik>\2$<pgr>', formatted_element)
                    split_paragraph = formatted_element.split('$')

                    inline_blocks = []
                    for inline in split_paragraph:
                        stripped_inline = re.sub(r'<\w\w\w>', '', inline)
                        if not inline.startswith('<pgr>'):
                            #Keep the spaces
                            stripped_inline = stripped_inline.strip()
                        ...
                        if inline.startswith('<pgr>'):
                            ...
                            try:
                                full_url = self.resolve_link(stripped_inline)
                            except NotionProcessorError as e:
                                logger.error(f'Error resolving URL "{stripped_inline}": {e}')
                                continue  # Skip invalid URLs

                            if full_url:
                                possessive = None
                                if '.' not in stripped_inline:
                                    possessive = '\' website' if stripped_inline.lower().endswith('s') else '\'s website'

                                content = stripped_inline + (possessive if possessive else '')
                                inline_block = {
                                    "type": "text",
                                    "text": {
                                        "content": content,
                                ...
                                logger.warning(f'Invalid website URL: {stripped_inline}')

                        elif inline.startswith('<wik>'):
                            # Assuming <wik> is intended for Wikipedia links
                            try:
                                #Cut content_to_process to give context, not too long
                                whole_context = re.sub(r'<\w{3}>', '', re.sub(r'(wik|wiki|wikipedia)\(([^)]+)\)', r'<wik>\2', ' '.join(content_to_process)))
                                context = self.string_window_cut(whole_context, stripped_inline, wrap='wiki(keyword)')

                                if context:
                                    chat_history = ['user: Teorema di Weierstrass', 'chatbot: it', 'user: Astronomy', 'chatbot: en']
                                    language, status_code = self.cohere_request(f'\"{stripped_inline}\".\n\nWhich language is this? Respond strictly only with two-letter format of the language ("it", "en", "es", "de"...). If you aren\'t able to deduct a language, default to \"en\".', chat_history=chat_history)
                                else:
                                    language = 'en'

                                wiki_response = self.check_wikipedia_page(stripped_inline, language=language)

                                if wiki_response:
                                    if isinstance(wiki_response, list):
                                        if context:
                                            chat_history = [f'user: \"{context}\".\n\nThis is a paragraph. Identify the wiki text entry. Once you identified them, I will give you some links and you will have to decide which link to assign, based on the paragraph context',f'chatbot: The wiki text entry is wiki({stripped_inline}). Provide the links and I will choose which link could correspond to the wiki text entry, based on the context.']

                                            prompt = f'Here are the links: \"{wiki_response}\".\nBased on the context, which of these links you think corresponds to the text entry? Respond strictly only with a number, corresponding to the position of the item in the list you choose.'

                                            chosen_wiki_url_index, status_code = self.cohere_request(prompt, chat_history)
                                            wiki_url = wiki_response[int(chosen_wiki_url_index) - 1]
                                        else:
                                            prompt = f'Here are some wiki links: \"{wiki_response}\".\nWhich of these links you think corresponds to the text entry \"{stripped_inline}\"? Respond strictly only with a number, corresponding to the position of the item in the list you choose.'

                                            chosen_wiki_url_index, status_code = self.cohere_request(prompt)
                                            wiki_url = wiki_response[int(chosen_wiki_url_index) - 1]
                                    else:
                                        wiki_url = wiki_response
                                    ...
                                    for pairs in self.wiki_footnote_pairs:
                                        if pairs[2] == wiki_url:
                                            self.wiki_count -= 1
                                            superscript_number = pairs[1]
                                            break

                                    if not superscript_number:
                                        superscript_number = ''.join(self.superscript_mapping[digit] for digit in str(self.wiki_count))
                                        self.wiki_footnote_pairs.append([stripped_inline,superscript_number,wiki_url])
                                        self.wiki_count += 1
                                    ...
                                    superscript = {
                                        ...
                                        "plain_text": superscript_number,
                                        "href": None
                                        }
                                    
                                    inline_blocks.append(superscript)
                                    ...
                                    inline_blocks.append(invalid_superscript)
                                    logger.warning(f"No Wikipedia page found for '{stripped_inline}' while processing '<wik>' marker.")

                            except NotionProcessorError as e:
                                logger.error(f'Error fetching Wikipedia link for "{stripped_inline}": {e}')
                                continue  # Skip if Wikipedia link cannot be resolved
                    ...
                    raise SyntaxError(
                        message=f'Unrecognized element marker in "{element}"',
                        location='Content Processor',
                        troubleshoot='Check your content formatting.'
                    )
            except NotionProcessorError as e:
                # Already logged in the exception class
                raise e
            except Exception as e:
                logger.error(f"Unexpected error in notion_api_children: {e}")
                raise SyntaxError(
        ...
        logger.info(f"Processed {len(children)} child block(s).")
        return children
    ...
    def process_and_post(
        ...
        try:
            ...
            if not content_to_process:
                ...
                if wiki_footnote == 'true':
                    sub_children = []
                    for name_link_pair in self.wiki_footnote_pairs:
                        nrp = {
                            'object': 'block',
                            'type': 'numbered_list_item',
                            'numbered_list_item': {
                                'rich_text': [
                                    {
                                        "type": "text",
                                        "text": {
                                            "content": name_link_pair[0],
                                            "link": {
                                                "url": name_link_pair[2]
                                            }
                                        },
                                        "annotations": {
                                            "bold": False,
                                        ...
                                        "plain_text": name_link_pair[0],
                                        "href": name_link_pair[2]
                                    },
                                    {
                                        "type": "text",
                                        "text": {
                    ...
                    if status_code == 200:
                        ...
                        footnote_ids = [obj.get('id') for obj in response.get('results')]
                        links = [f'https://www.notion.so/{page_posted_id.replace("-", "")}#{id.replace("-", "")}' for id in footnote_ids]

                        updated_block_ids = self.assign_links_to_superscripts(page_posted_id, links)
                        links2 = [f'https://www.notion.so/{page_posted_id.replace("-", "")}#{id.replace("-", "")}' for id in updated_block_ids]

                        logger.debug(f"Updated links: {links2}")

                        self.update_anchor_links(quote_id, links2)
                    else:
            ...
            raise APIError(
                f'Unexpected error: {str(e)}',
                location='Process Input',
                troubleshoot='An unexpected error occurred.'
            ) from e

# Example usage within a web server context (e.g., Flask)
# This part is optional and provided for context on how to integrate with a web server.
...
#         )
#         return jsonify({'success': True, 'page_id': result['page_id']}), 201
#     except NotionProcessorError as e:
#         return jsonify(e.to_dict()), e.status_code
#     except Exception as e:
#         logger.exception("Unhandled exception in create_notion_page endpoint.")
#         return jsonify({
#             'error': {
#                 'type': 'InternalServerError',
#                 'message': 'An unexpected error occurred.',
#                 'location': 'Server',
#                 'troubleshoot': 'Contact support.'
#             }
#         }), 500
